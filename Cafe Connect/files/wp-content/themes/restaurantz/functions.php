...
if ( ! function_exists( 'restaurantz_blocks_support' ) ) :
	/**
	 * Create add default blocks support
	 */
	function restaurantz_blocks_support() {
		...
		add_theme_support(
			...
			array(
				array(
					'name'      => esc_html__( 'Small', 'restaurantz' ),
					'shortName' => esc_html__( 'S', 'restaurantz' ),
					'size'      => 14,
					'slug'      => 'small',
				),
				array(
					'name'      => esc_html__( 'Normal', 'restaurantz' ),
					'shortName' => esc_html__( 'M', 'restaurantz' ),
					'size'      => 18,
					'slug'      => 'normal',
				),
				array(
					'name'      => esc_html__( 'Large', 'restaurantz' ),
					'shortName' => esc_html__( 'L', 'restaurantz' ),
					'size'      => 42,
					'slug'      => 'large',
				),
				array(
					'name'      => esc_html__( 'Huge', 'restaurantz' ),
					'shortName' => esc_html__( 'XL', 'restaurantz' ),
					'size'      => 54,
					'slug'      => 'huge',
				),
			)
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'White', 'restaurantz' ),
				'slug'  => 'white',
				'color' => '#ffffff',
			),
			array(
				'name'  => esc_html__( 'Black', 'restaurantz' ),
				'slug'  => 'black',
				'color' => '#111111',
			),
			array(
				'name'  => esc_html__( 'Gray', 'restaurantz' ),
				'slug'  => 'gray',
				'color' => '#f4f4f4',
			),
			array(
				'name'  => esc_html__( 'Blue', 'restaurantz' ),
				'slug'  => 'blue',
				'color' => '#21759b',
			),
			array(
				'name'  => esc_html__( 'Yellow', 'restaurantz' ),
				'slug'  => 'yellow',
				'color' => '#ffc300',
			),
			array(
				'name'  => esc_html__( 'Brown', 'restaurantz' ),
				'slug'  => 'brown',
				'color' => '#562c19',
			),
		) );
	}
	add_action( 'after_setup_theme', 'restaurantz_blocks_support', 20 );
endif; //restaurantz_blocks_support

if ( ! function_exists( 'restaurantz_add_blocks_style' ) ) :
	/**
	 * Add Blocks Style
	 */
	function restaurantz_add_blocks_style() {
		// Theme block stylesheet.
		wp_enqueue_style( 'restaurantz-block-style', get_theme_file_uri( '/css/blocks.css' ), array( 'restaurantz-style' ), date( 'Ymd-Gis', filemtime( get_template_directory() . '/css/blocks.css' ) ) );
	}
	add_action( 'wp_enqueue_scripts', 'restaurantz_add_blocks_style' );
endif; //restaurantz_add_blocks_style

if ( ! function_exists( 'restaurantz_block_editor_styles' ) ) :
	/**
	 * Enqueue editor styles for Blocks
	 */
	function restaurantz_block_editor_styles() {
		// Block styles.
		wp_enqueue_style( 'restaurantz-block-editor-style', get_theme_file_uri( '/css/editor-blocks.css' ), null, date( 'Ymd-Gis', filemtime( get_template_directory() . '/css/editor-blocks.css' ) ) );

		// Add custom fonts.
		wp_enqueue_style( 'restaurantz-fonts', restaurantz_fonts_url(), array(), null );
	}
	add_action( 'enqueue_block_editor_assets', 'restaurantz_block_editor_styles' );
endif; //restaurantz_block_editor_styles
